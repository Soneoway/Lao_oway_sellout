...
$from                   = $this->getRequest()->getParam('from', date('d/m/Y', strtotime("-45 days")));
...
$itemModel                  = new Application_Model_NewPcFollow();
...
if ($export) {
    if ($export == 1) {
        $data = $itemModel->reportExcelNewPcFollow($params);

        $this->exportReportNewFcFollow($data, $params);
    }
}
// print_r($result);
...
$this->view->desc    = $desc;
$this->view->sort    = $sort;
$this->view->url     = HOST.'trainer/new-pc-follow/'.( $params ? '?'.http_build_query($params).'&' : '?' );
...
$this->_helper->viewRenderer->setRender('new-pc-follow/index');