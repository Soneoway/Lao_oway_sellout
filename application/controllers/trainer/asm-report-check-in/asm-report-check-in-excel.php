...
$list_sales_check_in = $QSalesCheckInLog->getSalesCheckIn($staff_id, $sales_check_in_detail['store_id'], $start_date, $end_date);
...
$sheet->setCellValue('B2', " " . $sales_check_in_detail['staff_code']);
$sheet->setCellValue('A3', "Staff Name : ");
$sheet->setCellValue('B3', $sales_check_in_detail['staff_name']);
$sheet->setCellValue('A4', "Start Date : ");
$sheet->setCellValue('B4', date('d/m/Y', strtotime($sales_check_in_detail['joined_at'])));
$sheet->setCellValue('A5', "Created User : ");
$sheet->setCellValue('B5', date('d/m/Y', strtotime($sales_check_in_detail['created_at'])));
$sheet->setCellValue('A6', "First Check In : ");
$sheet->setCellValue('B6', date('d/m/Y', strtotime($sales_check_in_detail['first_check_in'])));
$sheet->setCellValue('A7', "Off Date : ");
$sheet->setCellValue('B7', isset($sales_check_in_detail['off_date']) ? date('d/m/Y', strtotime($sales_check_in_detail['off_date'])) : '-');
    ...
    for ($i = 0; $i < $cnt_data; $i++) {
        ...
        if ($data[$i]['mode_code'] == "CHECKIN") {
            ...
            if (date('Y-m-d', strtotime($data[$i]['first_check_in'])) == date('Y-m-d', strtotime($data[$i]['day_week']))
                ...
                $remark_other = "เริ่มงานวันแรก";
            } else if (date('Y-m-d', strtotime($data[$i]['first_check_in'])) < date('Y-m-d', strtotime($sales_check_in_detail['first_leave'])) && date('Y-m-d', strtotime($data[$i]['first_check_in'])) == date('Y-m-d', strtotime($data[$i]['day_week']))) {
                $remark_other = "เข้าระบบครั้งแรก";
            }
            ...
            $sheet->setCellValue($alpha++ . $index, isset($data[$i]['area_name'])? ($data[$i]['area_name']) :"-");
            $sheet->setCellValue($alpha++ . $index, isset($data[$i]['store_name'])? "[".$data[$i]['store_id']."] ".$data[$i]['store_name']:
                "-");
            ...
            $sheet->setCellValue($alpha++ . $index, $remark_other.' '.$data[$i]['remark']);
        } elseif ($data[$i]['mode_code'] == "LEAVE") {
            $remark_other = "";
            $remark_leave = "";
            if ($data[$i]['action_id'] == 1) {
               if ($data[$i]['certificate_file'] != null) {
                   $remark_leave = "ป่วยมีใบแพทย์";
               } else {
                   $remark_leave = "ป่วยไม่มีใบแพทย์";
               }
            }
            if (date('Y-m-d', strtotime($data[$i]['first_check_in'])) == date('Y-m-d', strtotime($data[$i]['day_week']))
                && date('Y-m-d', strtotime($data[$i]['day_week'])) == date('Y-m-d', strtotime($sales_check_in_detail['joined_at']))
            ) {
                $remark_other = "เริ่มงานวันแรก";
            } else if (date('Y-m-d', strtotime($data[$i]['first_check_in'])) < date('Y-m-d', strtotime($sales_check_in_detail['first_check_in']))
            ...
            if (date('Y-m-d', strtotime($data[$i]['day_week'])) < date("Y-m-d")) {
                ...
                if (date('Y-m-d', strtotime($data[$i]['day_week'])) > $dateBegin && date('Y-m-d', strtotime($data[$i]['day_week'])) < date('Y-m-d', strtotime($sales_check_in_detail['joined_at']))) {
                    $remark = "";
                    if(date('Y-m-d', strtotime($data[$i]['day_week'])) == date('Y-m-d', strtotime($sales_check_in_detail['joined_at']))) {
                    ...
                    if (isset($sales_check_in_detail['off_date'])) {
                        if (date('Y-m-d', strtotime($data[$i]['day_week'])) >= date('Y-m-d', strtotime($sales_check_in_detail['off_date']))) {
                            if (date('Y-m-d', strtotime($data[$i]['day_week'])) == date('Y-m-d', strtotime($sales_check_in_detail['off_date']))) {
                                $sheet->setCellValue($alpha++ . $index, date("d/m/Y", strtotime($data[$i]['day_week'])));
                                $sheet->setCellValue($alpha++ . $index, DayOfWeek($data[$i]['day_week'], 1));
                                $sheet->setCellValue($alpha++ . $index, "วันที่มีผลลาออก (".$sales_check_in_detail['off_date'].")");
                                $sheet->setCellValue($alpha++ . $index, "");
                                $sheet->setCellValue($alpha++ . $index, "");
                                $sheet->setCellValue($alpha++ . $index, "");
                                $sheet->setCellValue($alpha++ . $index, "");
                            ...
                            $sheet->setCellValue($alpha++ . $index, "-");
                            $sheet->setCellValue($alpha++ . $index, "");
                            $sheet->setCellValue($alpha++ . $index, "ขาดงาน");
                            $sheet->mergeCells("C" . $index . ":F" . $index);
                            $sheet->getStyle("C" . $index . ":F" . $index)->applyFromArray($style);
                        }
                    } else {
...
$filename = 'Sales Check In Report-' . $sales_check_in_detail['staff_code'] . '-' . date('M', strtotime($start_date));
