...
if ($export && $export == 1) {
    ...
    $report_check_in = $QBmCheckInLog->loadReportExcelAllCheckIn($area_id, $start_date_export, $end_date_export);
   ...
   die;
    ...
    $this->bm_check_in_by_area_export($date_title, $report_check_in, $area_name);
}

$pc_check_in = $QBmCheckInLog->fetchPagination($page, $limit, $total, $params);

$params['get_total_count'] = 0;
$total_count = $QBmCheckInLog->fetchPagination(null, null, $total, $params);
...
$this->view->url = HOST.'trainer/bm-report-check-in/'.( $params ? '?'.http_build_query($params).'&' : '?' );
...
if($this->getRequest()->isXmlHttpRequest()) {
    ...
    $this->_helper->viewRenderer->setRender('bm-report-check-in/partials/list');
} else
    $this->_helper->viewRenderer->setRender('bm-report-check-in/index');