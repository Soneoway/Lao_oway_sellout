...
if (isset($staff_code) && $staff_code) {
    $get_staff_id = $QPcCheckInLog->getStaffId($staff_code);
...
if (!empty($month_year)) {
    $list_pc_check_in = $QPcCheckInLog->getPcCheckIn($staff_id, $store_id, $start_date, $end_date);
    ...
    for ($i=0; $i<=$date_range; $i++) {
        ...
        $check=$QPcCheckInLog->getRemarkGroup($staff_id, $start_date, $end_date);
...
$this->_helper->viewRenderer->setRender('/report-check-in/create');
