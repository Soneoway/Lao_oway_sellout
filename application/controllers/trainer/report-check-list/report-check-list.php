...
$itemModel                  = new Application_Model_ReportCheckList();

if (in_array($userStorage->group_id, My_Staff_Group::$allow_in_area_view) && !My_Staff_Permission_Area::view_all($userStorage->id))
    $params['asm'] = $userStorage->id;

$this->view->topic = $itemModel->getTopicCheckList();
...
if ($export) {
    if ($export == 1) {
        $data = $itemModel->reportExcelCheckList($params);
        $this->exportReportCheckList($data, $params);
    }
}
// print_r($result);
...
$this->view->desc    = $desc;
$this->view->sort    = $sort;
$this->view->url     = HOST.'trainer/report-check-list/'.( $params ? '?'.http_build_query($params).'&' : '?' );
...
$this->_helper->viewRenderer->setRender('report-check-list/index');