...
class TimingController extends My_Controller_Action
{
    ...
    public function timingControlAction()
    {
        require_once 'timing' . DIRECTORY_SEPARATOR . 'timing_control.php';
    }

    // Scan InTo Store

    public function scanIntoAction()
    {
        require_once 'timing' . DIRECTORY_SEPARATOR . 'scan-into' . DIRECTORY_SEPARATOR . 'scan_into.php';
    }

    public function addScanIntoAction()
    {
        require_once 'timing' . DIRECTORY_SEPARATOR . 'scan-into' . DIRECTORY_SEPARATOR . 'add_scan_into.php';
    }

    public function saveScanIntoAction()
    {
        require_once 'timing' . DIRECTORY_SEPARATOR . 'scan-into' . DIRECTORY_SEPARATOR . 'save_scan_into.php';
    }

    // End

    public function timingControlAddAction()
    {
     require_once 'timing' . DIRECTORY_SEPARATOR . 'timing_control_add.php'; 
 }

 public function timingControlSaveAction()
 {
    require_once 'timing' . DIRECTORY_SEPARATOR . 'timing_control_save.php';
}

public function deleteSaleZeorAction(){
    $QStoreStaff = new Application_Model_StoreStaff();
    $QStoreStaffLog = new Application_Model_StoreStaffLog();

    $where = $QStoreStaff->getAdapter()->quoteInto('staff_id =?',0);
    $check1 = $QStoreStaff->delete($where);

    $where2 = $QStoreStaffLog->getAdapter()->quoteInto('staff_id =?',0);
    $check2 = $QStoreStaffLog->delete($where2);

    echo '<script>parent.location.href="'.('/index' ).'"</script>';
    exit;
}

    //create invoice sellout
public function invoiceCreateAction()
{
    require_once 'timing' . DIRECTORY_SEPARATOR . 'create-invoice.php';
}

    //save create invoice
public function saveInvoiceAction(){
    require_once 'timing' . DIRECTORY_SEPARATOR . 'save-invoice.php';
}

    //invoice sellout print
public function printInvoiceAction(){
    require_once 'timing' . DIRECTORY_SEPARATOR . 'sellout-invoice.php';
}
...
public function deleteInvoiceAction(){
    ...
    $where = $QPrint->getAdapter()->quoteInto('id = ?', $id);
    $QPrint->delete($where);
...
public function reportTimingAction()
{
    require_once 'timing'.DIRECTORY_SEPARATOR.'report-timing'.DIRECTORY_SEPARATOR.'report-timing.php';
}

    //invoice ListView
public function invoiceAction(){
    require_once 'timing'.DIRECTORY_SEPARATOR.'invoice'.DIRECTORY_SEPARATOR.'invoice.php';
}

         //function preorder 
public function customerOrderAction()
{
   require_once 'timing' . DIRECTORY_SEPARATOR . 'customer-preorder.php';
}

         //function save preorder
public function savePreorderAction()
{
   require_once 'timing' . DIRECTORY_SEPARATOR. 'save-preorder.php';
}

         //confirm Preorder
public function preorderListAction()
{
   require_once 'timing' . DIRECTORY_SEPARATOR . 'preorder-list.php';
}

public function stockDetailAction()
{
   require_once 'timing' .DIRECTORY_SEPARATOR. 'distributor-doi' .DIRECTORY_SEPARATOR. 'stock-detail.php';
}

//function send 
public function addImeiAction()
{
   require_once 'timing' . DIRECTORY_SEPARATOR .'add-imei.php';
}

public function editPreAction(){
        $flashMessenger = $this->_helper->flashMessenger;
        ...
        $Goodcolor = $QGoodColorCombined->getColorByModelNew($good_id);
        ...
        $this->view->color = $Goodcolor;
        $this->view->preorder = $preorder;
        $this->view->store = $store;
        $this->view->params = $params;

        $flashMessenger->setNamespace('success')->addMessage('Done.');
        $this->_helper->viewRenderer->setRender('/preorder/edit-preorder');
    }

    public function saveEditPreAction()
    {
        $id = $this->getRequest()->getParam('pre_id');
        $good_color = $this->getRequest()->getParam('good_color');
        ...
        $where = $QCustromerPreOrder->getAdapter()->quoteInto('id =?',$id);
        $resualt = $QCustromerPreOrder->update($data,$where);
...
//delete Proder
public function deletePreAction()
{
   ...
   if(!$id){
       echo "<script>
       parent.alert('have not ID');
       window.location.href ='".('/timing/preorder-list' )."';
       </script>";
   }
   ...
   $QCustromerPreOrder->delete($where);
   ...
   exit;
          ...
          //rolback PreOrder
public function rolbackPreAction()
{
   $this->_helper->layout->disableLayout();
   $this->_helper->viewRenderer->setNoRender();
   ...
   if(!$imei_sn){
       echo "<script>
       parent.alert('Order Is Not Confirm Can`t Rolback.');
       window.location.href ='".('/timing/preorder-list' )."';
       </script>";
   }
   ...
   $where = $QTimingSale->getAdapter()->quoteInto('imei =?',$imei_sn);
   $timing_detail = $QTimingSale->fetchRow($where);
   $timing_id = $timing_detail['timing_id'];

   $QTiming = new Application_Model_Timing();

   $where2 = $QTiming->getAdapter()->quoteInto('id =?',$timing_id);
   $delete_timing = $QTiming->delete($where2);

   if($delete_timing){
       $where3 = $QTimingSale->getAdapter()->quoteInto('imei =?',$imei_sn);
       $delete_timing_sale = $QTimingSale->delete($where3);

       if($delete_timing_sale){
           $data =array(
               'imei' => null,
               'add_time' => null,
           );

           $where4 = $QCustromerPreOrder->getAdapter()->quoteInto('imei =?',$imei_sn);
           $QCustromerPreOrder->update($data, $where4);
       }
   }
...
//report Activate IMEI
public function analyticsImeiAction()
{
    require_once 'timing' . DIRECTORY_SEPARATOR . 'analytics-imei.php';
}
...
public function timingIssueSave2Action()
{
    require_once 'timing'.DIRECTORY_SEPARATOR.'timing-issue'.DIRECTORY_SEPARATOR.'timing-issue-save2.php';
}
...
private function checkImei($imei, $timing_sales_id, &$info, $tool = null, $reimport = false)
{
        ...
        // print_r($imei); exit;
                ...
                $checkImeiInSystem = $QImei->fetchRow($where);
        
        if(!$checkImeiInSystem) {
            return 2;
        }
        ...
        if ($ts)
        {
            ...
            $brands = $QBrand->getBrand($wh_result['good_id']);
            ...
            $dis_imei = $QDistributor->find($wh_result['distributor_id']);
            $distributor_name = $dis_imei->current();
            ...
            $QArea = new Application_Model_Area();
            $area_cached = $QArea->get_cache();
            $regional_market_cached = $QRegionalMarket->get_cache();

            // get Area name by Khuan
            $QRegionalMarket = new Application_Model_RegionalMarket();
            $where_rm = $QRegionalMarket->getAdapter()->quoteInto('id =?',$distributor_name['region']);
            $result_rm = $QRegionalMarket->fetchRow($where_rm);

            $QTimingControl = new Application_Model_TimingControl();
            $check_control = $QTimingControl->getAdapter()->quoteInto('imei =?',$imei);
            $result_control = $QTimingControl->fetchRow($check_control);
                ...
                // }
            if($result_control){
                $info['status_control'] = "YES";
            }else{
                $info['status_control'] = "NO";
            }
            ...
            if($distributor_name) {
                $info['distributor_name'] = $distributor_name['title'];
            }

            $info['area_name'] = $area_cached[$result_rm['area_id']];
            $info['provice_name'] = $regional_market_cached[$distributor_name['region']];

            if ($store) {
                $info['store_code'] = $store['store_code'];
                $info['store'] = $store['name'];
            }
        ...
        // Add by Sak
                ...
                if ($result) {
                    ...
                    if($result['distributor_id'] != ''){
                        ...
                        if($result['activated_date']){
                            $info['active_date'] = $result['activated_date'];
                        }
                    ...
                    // return 12;
                        ...
                        if(!$result['distributor_id']){
                            $info['distributor_name'] = $warehouse_cached[$result['warehouse_id']];
                        }
                ...
                // if($result['type'] == 3) { // staff
                //     return 50;
                // }

                // if($result['type'] == 5) { //apk
                //     return 51;
                // }
 ...
 // Export Report By Store : Export CSV List [Same As List] 
    private function _exportExcelStoreList($result,$params) {
        ...
        $filename = 'Report_By_Store_list_'.date('d/m/Y');
        // output headers so that the file is downloaded rather than displayed
        header('Content-Type: text/csv; charset=utf-8');
        ...
        foreach ($result as $data) {
            ...
            // PCDB
            $staff_list = $QStoreStaff->getPCDB($data['st_id'],0);
            $cnt = count($staff_list);
            // $row[] = $cnt;
            $pcdb_name = "";
            $pcdb_code = "";
            ...
            if ($cnt > 0 ) {
                for ($j=0;$j<$cnt;$j++) {
                    if ($j != $cnt-1) { 
                        $pcdb_name = $pcdb_name.$staff_list[$j]['pcdb_name']." / ";  
                        $pcdb_code = $pcdb_code.$staff_list[$j]['pcdb_code']." / "; 
                    } else { 
                        $pcdb_name = $pcdb_name.$staff_list[$j]['pcdb_name'];
                        $pcdb_code = $pcdb_code.$staff_list[$j]['pcdb_code']; 
                    }
                }
            } else { $pcdb_name = "-"; $pcdb_code = "-"; }
    ...
    private function _exportExcelArea($data) {
        ...
        for ($i=0;$i<count($data); $i++) {
            ...
            $sellout = $data[$i]['sellout'] + $data[$i]['oppo_sellout'];
            $activate = $data[$i]['oppo_activated'] + $data[$i]['realme_activated'];
            ...
            $sheet->setCellValue($alpha++.$index, $activate);
...
// Export Report By Product : Export Excel
    private function _exportExcelModel($data) {
        require_once 'PHPExcel.php';
        $PHPExcel = new PHPExcel();
        $heads = array(
            'No.',
        ...
        for ($i=0; $i < count($data) ; $i++) { 
            ...
            // $sheet->setCellValue($alpha++.$index, $data[$i]['st_oppo_id']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['store_name']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['d_code']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['d_name']);
            // $sheet->setCellValue($alpha++.$index, $data[$i]['d_warehouse']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['brand_name'].' '.$data[$i]['product_name']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['total']);
            $index++;
        }

        $filename = 'Product_Model_list_'.date('d/m/Y');
        $objWriter = new PHPExcel_Writer_Excel2007($PHPExcel);
    ...
    // Export Report By Product : Export Model
    private function _exportExcelProduct($data) {
        ...
        for ($i=0;$i<count($data); $i++) {
            ...
            $alpha    = 'A';
            $sheet->setCellValue($alpha++.$index, $i+1);
            $sheet->setCellValue($alpha++.$index, $data[$i]['reporter_code']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['reporter_name']);
            ...
            //$sheet->setCellValue($alpha++.$index, $data[$i]['province']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['brand_name'].' '.$data[$i]['product_name']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['brand_name'].' '.$data[$i]['product_desc']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['color_name']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['total']);
            $index++;
        }
        
        $filename = 'Product_Excel_list_'.date('d/m/Y');
        $objWriter = new PHPExcel_Writer_Excel2007($PHPExcel);
    ...
    private function _exportExcelAchieveRemark($resualt){
        set_time_limit(0);
        error_reporting(0);
        ini_set('display_error', 0);
        ini_set('memory_limit', -1);
        ...
        $heads = array(
            ...
            'To Day Sell',
            'Monthly Sell',
            'To Day Hero Product Sell',
            'Monthly Hero Product Sell',
            'Target All model',
            'Target Hero Product',
            'Sale Amount To Day ALL MODEL',
            'Sale Amount Of This Month ALL MODEL',
            'Sale Amount To Day Hero Product',
            'Sale Amount Of This Month Hero Product',
            'To Day Score',
        );
        ...
        foreach($resualt as $item){
            ...
            $row[] = $item['area_name'];
            $row[] = $item['score'];
            $row[] = $item['to_day_sell'];
            $row[] = $item['monthly_sell'];
            $row[] = $item['to_day_hero_sell'];
            $row[] = $item['monthly_hero_sell'];
            $row[] = $item['all_model_target'];
            $row[] = $item['hero_model_target'];
            $row[] = number_format($item['sell_amount_to_day_all_model']);
            $row[] = number_format($item['sell_amount_of_monthly_all_model']);
            $row[] = number_format($item['sell_amount_to_day_hero']);
            $row[] = number_format($item['sell_amount_of_monthly_hero']);

            // $total_sell = $total_sell + $item['to_day_sell'];
            // $to_day_score = (($item['to_day_sell'] * 60) / ( $item['score'] * $total_sell ) * 100 );
    ...
    private function _exportExcelRmGFK($params) {
        ...
        for ($i=0;$i<count($rm_list); $i++) {
            ...
            $sheet->setCellValue($alpha++.$index, $rm_list[$i]['staff_code']);
            ...
            $sheet->setCellValue($alpha++.$index, number_format($gfk,2)."%");
            $sheet->setCellValue($alpha++.$index, number_format($sellout));
            $sheet->setCellValue($alpha++.$index, number_format($total_sellout));
        ...
        $filename = 'Score_Report_RD_GFK_'.date('Y-m-d_H:i:s');
        $objWriter = new PHPExcel_Writer_Excel2007($PHPExcel);
    ...
    private function _exportDOIList($data) {
        ...
        for ($i=0;$i<count($data); $i++) {
            ...
            $QDistributor = new Application_Model_Distributor();
            $superi = $QDistributor->getSuperiorDistributor($data[$i]['warehouse_id']);

            $alpha    = 'A';
            $sheet->setCellValue($alpha++.$index, $data[$i]['area_name']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['province_name']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['sales_area']);
            $sheet->setCellValue($alpha++.$index, $superi[0]['title']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['distributor_code']);
...
// Export DOI IMEI
    public function _exportDOIImeiList($data){
       ...
       $QCategories = new Application_Model_GoodCategory();
       $QDistributor = new Application_Model_Distributor();


       $Gname = $QGood->get_cache();
       $Cname = $QColor->get_cache();
       $Category = $QCategories->get_cache();
    ...
    for ($i=0;$i<count($data); $i++) {
        ...
        $superi = $QDistributor->getSuperiorDistributor($data[$i]['warehouse_id']);
            ...
            $model_type = "-";
            if ($data[$i]['model_type'] == 1) { $model_type = "Normal"; }
            if ($data[$i]['model_type'] == 3) { $model_type = "Staff"; }
            if ($data[$i]['model_type'] == 5) { $model_type = "DEMO"; }
        ...
        $sheet->setCellValue($alpha++.$index, $data[$i]['store_code']);
        $sheet->setCellValue($alpha++.$index, $data[$i]['store_name']);
        $sheet->setCellValue($alpha++.$index, $Category[$data[$i]['category']]);
        $sheet->setCellValue($alpha++.$index, $data[$i]['brand_name'].' '.$Gname[$data[$i]['good']]);
        $sheet->setCellValue($alpha++.$index, $Cname[$data[$i]['color']]);
        $sheet->setCellValue($alpha++.$index, $data[$i]['imeisn']);
        $sheet->setCellValue($alpha++.$index, $model_type);
        $sheet->setCellValue($alpha++.$index, $data[$i]['instock']);
        $sheet->setCellValue($alpha++.$index, $data[$i]['activated_date']);     
...
public function _exportDOIModelList($data){
...
$filename = 'Distributor_DOI_Model_List_'.date('d/m/Y');
$objWriter = new PHPExcel_Writer_Excel2007($PHPExcel);
ob_end_clean();
...
*/
    ...
    // export imei store
    private function _exprotbystoreImei($result,$params){
        ...
        $filename = 'Report_By_Store_Imei_'.date('d/m/Y');
        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename='.$filename.'.csv');
        ...
        foreach ($result as $data) {
           ...
           // $row[] = $data['st_name'];
            ...
            $row[] = $staff[$data['reporter_name']];
            $row[] = $data['brand_name']." ".$good[$data['product_id']];
            $row[] = $color[$data['color_id']];
            $row[] = $data['imei_sn'];
	        $row[] = $data['remark'];
    ...
    private function _exportExcelImei($data) {
        set_time_limit(0);
        error_reporting(0);
        ini_set('display_error', 0);
        ini_set('memory_limit', -1);

        $filename = 'Analytics_By_Activated'.date('d/m/Y');
        // output headers so that the file is downloaded rather than displayed
        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename='.$filename.'.csv');
        // echo "\xEF\xBB\xBF"; // UTF-8 BOM
        ...
        foreach ($data as $item) {
            ...
            $row[] = $item['total'];
            $row[] = $item['activate_report'];
            $row[] = $item['activated_not_report'];
    ...
    private function _exportExcelImeiProduct($data){
        set_time_limit(0);
        error_reporting(0);
        ini_set('display_error', 0);
        ini_set('memory_limit', -1);

        $filename = 'Report_By_Product_Not_Timing'.date('d/m/Y');
        // output headers so that the file is downloaded rather than displayed
        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename='.$filename.'.csv');
        // echo "\xEF\xBB\xBF"; // UTF-8 BOM
    ...
    private function _exportExcelImeiDistridutor($data){
        set_time_limit(0);
        error_reporting(0);
        ini_set('display_error', 0);
        ini_set('memory_limit', -1);

        $filename = 'Report_By_Imei_Store_Not_Timing'.date('d/m/Y');
        // output headers so that the file is downloaded rather than displayed
        // header('Content-Type: text/csv; charset=UTF-8');
        // header('Content-Disposition: attachment; filename='.$filename.'.csv');
    ...
    private function _exportExcelImeiWarehouse($data){
        set_time_limit(0);
        error_reporting(0);
        ini_set('display_error', 0);
        ini_set('memory_limit', -1);

        $filename = 'Report_By_Imei_Warehouse_Not_Timing'.date('d/m/Y');
        // output headers so that the file is downloaded rather than displayed
        // header('Content-Type: text/csv; charset=UTF-8');
        // header('Content-Disposition: attachment; filename='.$filename.'.csv');
    ...
    function _exportPreOrder($data){
      require_once 'PHPExcel.php';
        ...
        for ($i=0;$i<count($data); $i++) {
            ...
            $sheet->setCellValue($alpha++.$index, $data[$i]['good_name']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['good_color']);
            $sheet->setCellValue($alpha++.$index, $type);
            $sheet->setCellValue($alpha++.$index, $data[$i]['customers_name']);
            $sheet->setCellValue($alpha++.$index, $gender);
            $sheet->setCellValue($alpha++.$index, $age);
            $sheet->setCellValue($alpha++.$index, $data[$i]['customers_phone']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['num']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['staff_code']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['staff_name']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['staff_position']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['add_time']);
            $index++;
        }
        
        $filename = 'Export_PreOrder_List_'.date('d/m/Y');
        $objWriter = new PHPExcel_Writer_Excel2007($PHPExcel);
        ob_end_clean();
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="' . $filename . '.xlsx"');
    ...
    private function _exportreporttiming($data){
        require_once 'PHPExcel.php';
        $PHPExcel = new PHPExcel();
        $heads = array(
            'No.',
            'All Model Sellout [ TODAY ]',
            'All Model Sellout [ Month ]',
            'All model Target',
            'Percentact [ All Model]',
            'Hero Product Sellout [ TODAY ]',
            'Hero Product Sellout [ Month ]',
            'Hero Product Target',
            'Percentact [ HeroProduct ]'
        );
        ...
        for ($i=0;$i<count($data); $i++) {
            ...
            $alpha    = 'A';
            $sheet->setCellValue($alpha++.$index, $i+1);
            $sheet->setCellValue($alpha++.$index, $data[$i]['to_day_allmodel']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['allmodel_sellout']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['target']);
            $sheet->setCellValue($alpha++.$index, 0);
            $sheet->setCellValue($alpha++.$index, $data[$i]['to_day_hero']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['hero_sellout']);
        ...
        $filename = 'Report_timing'.date('d/m/Y');
        $objWriter = new PHPExcel_Writer_Excel2007($PHPExcel);
 ...
 private function _exportreportAreatiming($data)
    {
        require_once 'PHPExcel.php';
        $PHPExcel = new PHPExcel();
        $heads = array(
            'No.',
            'Area Name',
            'TODAY SELLOUT [ All Model]',
            'THIS MONTH SELLOUT [ All Model]',
            'TARGET THIS MONTH [ All Model]',
            'PERCENTAGE (%) [ All Model]',
            'HTODAY SELLOUT [ Hero Product ]',
            'THIS MONTH SELLOUT [ Hero Product ]',
            'TARGET THIS MONTH [ Hero Product ]',
            'PERCENTAGE (%) [ Hero Product ]'
        );
        ...
        $index = 2;
        for ($i=0;$i<count($data); $i++) {
          $allmodel_sellout_percent =$data[$i]['allmodel_sellout'] * 100 / $data[$i]['target'];
          $hero_sellout_percent =$data[$i]['hero_sellout'] * 100 / $data[$i]['target_hero'];
          ...
          $sheet->setCellValue($alpha++.$index, $data[$i]['allmodel_sellout']);
          $sheet->setCellValue($alpha++.$index, $data[$i]['target']);
          $sheet->setCellValue($alpha++.$index, number_format($allmodel_sellout_percent,2).' %');
          $sheet->setCellValue($alpha++.$index, $data[$i]['to_day_hero']);
          $sheet->setCellValue($alpha++.$index, $data[$i]['hero_sellout']);
          $sheet->setCellValue($alpha++.$index, $data[$i]['target_hero']);
          $sheet->setCellValue($alpha++.$index, number_format($hero_sellout_percent,2).' %');
	  $index++;

        }
        
        $filename = 'Report_timing_Daily_By_Area_'.date('d/m/Y');
        $objWriter = new PHPExcel_Writer_Excel2007($PHPExcel);
...
// Report Daily Timing
private function _exportOppoReport($data,$oppo_hero_report,$model_report,$area_model_report,$oppo_score_report){
        require_once 'PHPExcel.php';
        require_once 'PHPExcel/IOFactory.php';
    
        $PHPExcel = new PHPExcel();
    
        $date = date('d-F',strtotime("-1 days"));
	...
	if($now_date == "1") {
        ...
        if($now_date-1 == 0){
            ...
            $yesterday = $now_date-1;
            $time_progress = number_format((($yesterday/$end_date)*100),2).'%';
        }
        ...
        $heads = array(
            ...
            $date,
            'Finished',
            ''.$month.' 月总目标 Target of '.$month_text.'',
            'Completion Rate',
            'Time Progress'
        );
        ...
        $head_style_sheet1 = array( // Header Style
            'alignment' => array(
                'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
                'vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,
            ),
        ...
        $head_style_sheet2 = array( // Header Style
            'alignment' => array(
                'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
                'vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,
            ),
        ...
        $thead_style1 = array( // General Theader Style
            'alignment' => array(
                'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
                'vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,
            ),
        ...
        $thead_style2 = array( // Hero Theader Style
            'alignment' => array(
                'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
                'vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,
            ),
        ...
        $head_style_sheet5 = array( // Header Style
            'alignment' => array(
                'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
                'vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,
            ),
        ...
        $body_cell5_style_sheet5 = array(
            'fill' => array(
                'type' => PHPExcel_Style_Fill::FILL_SOLID,
                'color' => array('rgb' => '22C50C')
            ),
            'font'  => array(
                'color' => array('rgb' => 'FFFFFF'),
            )
        );
        $body_cell6_style_sheet5 = array(
            'fill' => array(
                'type' => PHPExcel_Style_Fill::FILL_SOLID,
                'color' => array('rgb' => '1D19B7')
            ),
            'font'  => array(
                'color' => array('rgb' => 'FFFFFF'),
            )
        );
        $body_cell7_style_sheet5 = array(
            'fill' => array(
                'type' => PHPExcel_Style_Fill::FILL_SOLID,
                'color' => array('rgb' => '069B87')
            ),
            'font'  => array(
                'color' => array('rgb' => 'FFFFFF'),
            )
        ...
        $footer_style = array( // All Footer Style
            'alignment' => array(
                'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
                'vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,
            ),
        ...
        $PHPExcel->setActiveSheetIndex(0);
        $sheet    = $PHPExcel->getActiveSheet();
        $sheet->setTitle('General Target');
    
        // Sheet 2
        $PHPExcel->createSheet();
        $PHPExcel->setActiveSheetIndex(1);
        $sheet2   = $PHPExcel->getActiveSheet();
        $sheet2->setTitle('Model Report');

        // Sheet 3
        $PHPExcel->createSheet();
        $PHPExcel->setActiveSheetIndex(2);
        $sheet3   = $PHPExcel->getActiveSheet();
        $sheet3->setTitle('Hero Target');

        // Sheet 4
        $PHPExcel->createSheet();
        $PHPExcel->setActiveSheetIndex(3);
        $sheet4   = $PHPExcel->getActiveSheet();
        $sheet4->setTitle('Area Model Report');

        // Sheet 5
        $PHPExcel->createSheet();
        $PHPExcel->setActiveSheetIndex(4);
        $sheet5   = $PHPExcel->getActiveSheet();
        $sheet5->setTitle('Score Report');
        ...
        $sheet->setCellValue('B2','OPPO General Target');
        $sheet->getStyle("B2:G2")->applyFromArray($head_style);
        $sheet->getStyle("B3:G3")->applyFromArray($thead_style1);
    
        // Header General 2
        $sheet->mergeCells('J2:L2');
        $sheet->setCellValue('J2','2025年-'.$month.'月 总目标 General Target of '.$month_text.'');
        $sheet->getStyle("J2:L2")->applyFromArray($head_style_sheet1);
        $sheet->getStyle("J3:L3")->applyFromArray($thead_style1);
    
        // Footer General 1
        $sheet->setCellValue("B15","Total");
        $sheet->setCellValue("G15",$time_progress);
        $sheet->getStyle("B15:G15")->applyFromArray($footer_style);
    
        // Footer General 2
        $sheet->setCellValue("J15","Total");
        $sheet->setCellValue("L15",$time_progress);
        $sheet->getStyle("J15:L15")->applyFromArray($footer_style);


        // Header Hero 1
        $sheet3->mergeCells('B2:G2');
        $sheet3->setCellValue('B2','OPPO Hero Target of Reno13 Series');
        $sheet3->getStyle("B2:G2")->applyFromArray($head_style);
        $sheet3->getStyle("B3:G3")->applyFromArray($thead_style2);
    
        // Header Hero 2
        $sheet3->mergeCells('J2:L2');
        $sheet3->setCellValue('J2','2025年-'.$month.'月 OPPO Hero Target of Reno13 Series');
        $sheet3->getStyle("J2:L2")->applyFromArray($head_style_sheet2);
        $sheet3->getStyle("J3:L3")->applyFromArray($thead_style2);
    
        // Footer Hero 1
        $sheet3->setCellValue("B15","Total");
        $sheet3->setCellValue("G15",$time_progress);
        $sheet3->getStyle("B15:G15")->applyFromArray($footer_style);
    
        // Footer Hero 2
        $sheet3->setCellValue("J15","Total");
        $sheet3->setCellValue("L15",$time_progress);
        $sheet3->getStyle("J15:L15")->applyFromArray($footer_style);
        ...
        for($i = 4; $i < 15; $i++){
            ...
            $sheet->getStyle("$cel1:$cel2")->applyFromArray($body_style);
            $sheet->getStyle("$cel3:$cel4")->applyFromArray($body_style);
            $sheet3->getStyle("$cel1:$cel2")->applyFromArray($body_style);
            $sheet3->getStyle("$cel3:$cel4")->applyFromArray($body_style);
        }
        ...
        // Head Table 1 General
        foreach($heads as $key) {
            $sheet->setCellValue($alpha.$index, $key);
            $sheet3->setCellValue($alpha.$index, $key);
            $alpha++;
        }
    
        $alpha2 = 'J';
    
        // Head Table 2 General
        foreach($heads2 as $key) {
            $sheet->setCellValue($alpha2.$index, $key);
            $sheet3->setCellValue($alpha2.$index, $key);
            $alpha2++;
        }
        ...
        for ($i=0;$i<count($data); $i++) {
            ...
            $total_finished = $data[$i]['total_sale'] + $total_finished;
            $total_target = $data[$i]['general_target'] + $total_target;
            $total_yesterday_sale = $data[$i]['yesterday_sale'] + $total_yesterday_sale;
    
            $completion_rate = number_format((($data[$i]['total_sale'] / $data[$i]['general_target'])*100),2).'%';
            $total_completion_rate = number_format((($total_finished / $total_target)*100),2).'%';

            $sheet->setCellValue("D15",$total_finished);
            $sheet->setCellValue("E15",$total_target);
            $sheet->setCellValue("C15",$total_yesterday_sale);
            $sheet->setCellValue("F15",$total_completion_rate);
    
            $objConditional = new PHPExcel_Style_Conditional();
            $objConditional->setConditionType(PHPExcel_Style_Conditional::CONDITION_CELLIS)
            ->setOperatorType(PHPExcel_Style_Conditional::OPERATOR_LESSTHAN)
            ->addCondition($total_completion_rate)
            ->getStyle()
            ->applyFromArray($less_than);
    
            $conditionalStyles = $sheet->getStyle('F'.$index)->getConditionalStyles();
            array_push($conditionalStyles,$objConditional);
            $sheet->getStyle('F'.$index)->setConditionalStyles($conditionalStyles);
    
            $alpha    = 'B';
            $sheet->setCellValue($alpha++.$index, $data[$i]['area_name']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['yesterday_sale']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['total_sale']);
            $sheet->setCellValue($alpha++.$index, $data[$i]['general_target']);
            $sheet->setCellValue($alpha++.$index, $completion_rate);
            $sheet->setCellValue($alpha++.$index, $time_progress);
        ...
        for ($i=0; $i < count($data); $i++) {
          ...
          $sheet->setCellValue("K15",$total_completion_rate);
          ...
          $conditionalStyles2 = $sheet->getStyle('K'.$index2)->getConditionalStyles();
          array_push($conditionalStyles2,$objConditional2);
          $sheet->getStyle('K'.$index2)->setConditionalStyles($conditionalStyles2);
    
          $alpha    = 'J';
          $sheet->setCellValue($alpha++.$index2, $data[$i]['area_name']);
          $sheet->setCellValue($alpha++.$index2, $completion_rate);
          $sheet->setCellValue($alpha++.$index2, $time_progress);
...
// Start Hero
    ...
    // Head Table 1 Hero
        ...
        // Table 1 Hero
        $index = 4;
        for ($i=0;$i<count($oppo_hero_report); $i++) {
    
            $total_hero_finished = $total_hero_finished + $oppo_hero_report[$i]['total_hero_sale'];
            $total_hero_target = $total_hero_target + $oppo_hero_report[$i]['hero_target'];
            $total_hero_yesterday_sale = $total_hero_yesterday_sale + $oppo_hero_report[$i]['yesterday_hero_sale'];
    
            $completion_hero_rate = number_format((($oppo_hero_report[$i]['total_hero_sale'] / $oppo_hero_report[$i]['hero_target'])*100),2).'%';
            $total_hero_completion_rate = number_format((($total_hero_finished / $total_hero_target)*100),2).'%';


            $sheet3->setCellValue("D15",$total_hero_finished);
            $sheet3->setCellValue("E15",$total_hero_target);
            $sheet3->setCellValue("C15",$total_hero_yesterday_sale);
            $sheet3->setCellValue("F15",$total_hero_completion_rate);
    
            $objConditional = new PHPExcel_Style_Conditional();
            $objConditional->setConditionType(PHPExcel_Style_Conditional::CONDITION_CELLIS)
            ->setOperatorType(PHPExcel_Style_Conditional::OPERATOR_LESSTHAN)
            ->addCondition($total_hero_completion_rate)
            ->getStyle()
            ->applyFromArray($less_than);
    
            $conditionalStyles = $sheet3->getStyle('F'.$index)->getConditionalStyles();
            array_push($conditionalStyles,$objConditional);
            $sheet3->getStyle('F'.$index)->setConditionalStyles($conditionalStyles);
    
            $alpha    = 'B';
            $sheet3->setCellValue($alpha++.$index, $oppo_hero_report[$i]['area_name']);
            $sheet3->setCellValue($alpha++.$index, $oppo_hero_report[$i]['yesterday_hero_sale']);
            $sheet3->setCellValue($alpha++.$index, $oppo_hero_report[$i]['total_hero_sale']);
            $sheet3->setCellValue($alpha++.$index, $oppo_hero_report[$i]['hero_target']);
            $sheet3->setCellValue($alpha++.$index, $completion_hero_rate);
            $sheet3->setCellValue($alpha++.$index, $time_progress);
        ...
        for ($i=0; $i < count($oppo_hero_report); $i++) {
          ...
          $sheet3->setCellValue("K15",$total_hero_completion_rate);
          ...
          $conditionalStyles2 = $sheet3->getStyle('K'.$index2)->getConditionalStyles();
          array_push($conditionalStyles2,$objConditional2);
          $sheet3->getStyle('K'.$index2)->setConditionalStyles($conditionalStyles2);
    
          $alpha    = 'J';
          $sheet3->setCellValue($alpha++.$index2, $oppo_hero_report[$i]['area_name']);
          $sheet3->setCellValue($alpha++.$index2, $completion_hero_rate);
          $sheet3->setCellValue($alpha++.$index2, $time_progress);
...
// End Hero
    ...
    //   Sheet2 Model Report
      ...
      $total_row = count($model_report) + 4;
    
      for($i = 4; $i < count($model_report) + 4; $i++){
        $cel1 = 'B'.$i;
        $cel2 = 'C'.$i;
        $cel3 = 'D'.$i;
    
        $sheet2->getStyle("$cel1")->applyFromArray($body_style_cell1_sheet2);
        $sheet2->getStyle("$cel2:$cel3")->applyFromArray($body_style_sheet2);
    }
    
    $row1 = 'B'.$total_row;
    $row2 = 'D'.$total_row;
    $total_row = 'C'.$total_row;
    
    $sheet2->getStyle("B3:D3")->applyFromArray($head_style);
    $sheet2->setCellValue($row1,"Total");
    $sheet2->getStyle("$row1:$row2")->applyFromArray($footer_style);
    ...
    foreach($sheet2_heads as $key) {
        $sheet2->setCellValue($alpha_sheet2.$index_sheet2, $key);;
        $alpha_sheet2++;
    }
    ...
    $sheet_index2 = 4;
    for ($i=0; $i < count($model_report); $i++) {
    
        $total_sale = $model_report[$i]['total_sale'] + $total_sale;
        $avg = number_format((($model_report[$i]['total_sale'] / $total_yesterday_sale)*100),2).'%';
    
        $sheet2->setCellValue($total_row,$total_sale);
    
        $alpha    = 'B';
        $sheet2->setCellValue($alpha++.$sheet_index2, $model_report[$i]['brand_name'].' '.$model_report[$i]['model_name']);
        $sheet2->setCellValue($alpha++.$sheet_index2, $model_report[$i]['total_sale']);
        $sheet2->setCellValue($alpha++.$sheet_index2, $avg);
...
$sheet4_heads = array(
  ...
  $total_row = count($area_model_report) + 2;

  for($i = 2; $i < count($area_model_report) + 2; $i++){
    $cel1 = 'A'.$i;
    $cel2 = 'B'.$i;
    $cel3 = 'C'.$i;

    $sheet4->getStyle("$cel1")->applyFromArray($body_style);
    $sheet4->getStyle("$cel2")->applyFromArray($body_style_cell2_sheet4);
    $sheet4->getStyle("$$cel3")->applyFromArray($body_style);
}
...
$sheet4->getStyle("A1:C1")->applyFromArray($head_style);

$alpha_sheet4    = 'A';
$index_sheet4    = 1;

for($col = 'A'; $col !== 'D'; $col++) {
    $sheet4->getColumnDimension($col)
    ->setAutoSize(true);
}

foreach($sheet4_heads as $key) {
    $sheet4->setCellValue($alpha_sheet4.$index_sheet4, $key);;
    $alpha_sheet4++;
}


$sheet_index4 = 2;
for ($i=0; $i < count($area_model_report); $i++) {

    $area_total_sale = $area_model_report[$i]['area_total_sale'] + $area_total_sale;

    $alpha4 = 'A';
    $sheet4->setCellValue($alpha4++.$sheet_index4, $area_model_report[$i]['area_name']);
    $sheet4->setCellValue($alpha4++.$sheet_index4, $area_model_report[$i]['brand_name'].' '.$area_model_report[$i]['model_name']);
    $sheet4->setCellValue($alpha4++.$sheet_index4, $area_model_report[$i]['area_total_sale']);
...
$sheet5_heads = array(
    '日期 Date',
    '机构 RGM',
    "当日打分 Today \r Average Score",
    '月累计打分 Monthly Average Score',
    '点位 Point',
    'Sales Amount of Today',
    'Sales Amount of Month',
);

$total_row = count($oppo_score_report) + 4;

for($i = 4; $i < count($oppo_score_report) + 4; $i++){
  $cel1 = 'B'.$i;
  $cel2 = 'H'.$i;

  $sheet5->getStyle("$cel1:$cel2")->applyFromArray($body_style_sheet5);
}
...
$sheet5->getStyle("B3:H3")->applyFromArray($head_style_sheet5);
$sheet5->getStyle("C4")->applyFromArray($body_cell4_style_sheet5);
$sheet5->getStyle("C5")->applyFromArray($body_cell5_style_sheet5);
$sheet5->getStyle("C6")->applyFromArray($body_cell6_style_sheet5);
$sheet5->getStyle("C7")->applyFromArray($body_cell7_style_sheet5);

$alpha_sheet5    = 'B';
$index_sheet5    = 3;

for($col = 'B'; $col !== 'I'; $col++) {
  $sheet5->getColumnDimension($col)
  ->setAutoSize(true);
}

foreach($sheet5_heads as $key) {
  $sheet5->setCellValue($alpha_sheet5.$index_sheet5, $key);;
  $alpha_sheet5++;
}
...
$sheet_index5 = 4;
for ($i=0; $i < count($oppo_score_report); $i++) {

    $point = $oppo_score_report[$i]['score_point'];
    $daily_price = $oppo_score_report[$i]['area_price'];
    $monthly_price = $oppo_score_report[$i]['national_price'];

    $total_point = $oppo_score_report[$i]['score_point'] + $total_point;
    $total_daily_price = $oppo_score_report[$i]['area_price'] + $total_daily_price;
    $total_monthly_price = $oppo_score_report[$i]['national_price'] + $total_monthly_price;

    $score_daily = ((($daily_price / $oppo_score_report[$i]['total_daily_price']) / $point) * 60) * 100;
    $score_monthly = ((($monthly_price / $oppo_score_report[$i]['total_monthly_price']) / $point) * 60) * 100;

    $alpha5 = 'B';
    $sheet5->setCellValue($alpha5++.$sheet_index5, $date.'-2024');
    $sheet5->setCellValue($alpha5++.$sheet_index5, $oppo_score_report[$i]['area_name']);
    $sheet5->setCellValue($alpha5++.$sheet_index5, number_format($score_daily,2));
    $sheet5->setCellValue($alpha5++.$sheet_index5, number_format($score_monthly,2));
    $sheet5->setCellValue($alpha5++.$sheet_index5, $point);
    $sheet5->setCellValue($alpha5++.$sheet_index5, number_format($daily_price),2);
    $sheet5->setCellValue($alpha5++.$sheet_index5, number_format($monthly_price),2);
    ...
    // Footer Score Sheet5
    $sheet5->mergeCells('B8:E8');
    $sheet5->setCellValue("B8","Total");
    $sheet5->setCellValue("F8",$total_point);
    $sheet5->setCellValue("G8",number_format($total_daily_price),2);
    $sheet5->setCellValue("H8",number_format($total_monthly_price),2);
    $sheet5->getStyle("B8:H8")->applyFromArray($footer_style);


$filename = 'Report_timing_Daily_'.strtotime(date('Y-m-d'));
$objWriter = new PHPExcel_Writer_Excel2007($PHPExcel);
