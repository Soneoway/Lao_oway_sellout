	...
	if ( $data['target'] !== '' ) { 
		...
		if ( isset( $pmt_id[ $temp['staff_id'][$i] ] ) && $pmt_id[ $temp['staff_id'][$i] ] ) {
			...
			if ( $old_target != $data['target'] ) {
				...
				$where = $QOppoPcmTarget->getAdapter()->quoteInto('id = ?', $id);
				$result = $QOppoPcmTarget->update($data,$where); 
			...
			$result = $QOppoPcmTarget->insert($data);
		...
		if ( isset($data2['target']) && $data2['target'] !== '' ) { 
			...
			if ( isset( $hero_pmt_id[ $temp2['staff_id'][$i] ] ) && $hero_pmt_id[ $temp2['staff_id'][$i] ] ) {
				...
				if ( $old_target != $data2['target_hero'] ) {
					...
					$where = $QOppoPcmTarget->getAdapter()->quoteInto('id = ?', $id);
					$result = $QOppoPcmTarget->update($data2,$where); 
...
echo '<script>parent.location.href="'.HOST.'manage/pcm-target/pcm-target?month_year='.$month_year.'"</script>';