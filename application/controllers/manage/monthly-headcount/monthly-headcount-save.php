...
try {
	...
	$where = array();
	$where[] = $QMHL->getAdapter()->quoteInto('area_name = ?', $area_name[0]);
	$where[] = $QMHL->getAdapter()->quoteInto('month_year = ?', $month_year[0]);
	$result = $QMHL->fetchAll($where)->ToArray();
	...
	if ( empty($result) ) {
		...
		for ($i=0;$i<count($staff_code);$i++) {
			...
			$result_insert = $db->insert('monthly_headcount_list', $data); 
		}
		...
		if ( !empty($chk_arr) ) {
			...
			$db->delete('monthly_headcount_list', $where_del);
		}
		...
		for ($i=0;$i<count($mhl_id);$i++) {
			...
			if ( $mhl_id[$i] == 0 ) {
				...
				$where = array();
        		$where['id = ?'] = $mhl_id[$i];
				$result_insert = $db->update('monthly_headcount_list', $data, $where); 
...
echo '<script>parent.location.href="'. ( !empty( $back_url ) ? ($back_url) : (HOST.'manage/monthly-headcount') ).'"</script>';