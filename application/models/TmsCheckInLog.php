...
class Application_Model_TmsCheckInLog extends Zend_Db_Table_Abstract
{
    protected $_name = 'tms_check_in_log';

    function fetchPagination($page, $limit, &$totaltms, $params)
    {
        ...
        $get = array(
            ...
            'area_name' => 'ar.name',
            'group_name'=> 'g.name',
             'type_report' => new Zend_Db_Expr("'TMS'"),
        );
       //  echo "<pre>";
       // print_r($get);
        $select = $db->select()
       ...
       // echo $select;
        ...
        if ($limit)
            $totaltms = $db->fetchOne("select FOUND_ROWS()");
  ...
  // echo $select;
    ...
    function getDetails($sf_id)
    {
        ...
        $get = array(
            ...
            'first_check_in' => new Zend_Db_Expr('(SELECT DATE(scl.created_at) FROM tms_check_in_log AS scl WHERE scl.staff_id = sf.id ORDER BY scl.created_at ASC LIMIT 1)'),
            'first_leave' => new Zend_Db_Expr('(SELECT DATE(sl.created_at) FROM tms_leave_log AS sl WHERE sl.staff_id = sf.id ORDER BY sl.created_at ASC LIMIT 1)'),
        );
...
//            ->where('sf.off_date IS NULL')
    ...
    function getSalesCheckIn($staff_id, $store_id, $start_date, $end_date)
    {
        ...
        $get = array(
            ...
            'joined_at' => 'sf.joined_at',
            'first_check_in' => new Zend_Db_Expr('(SELECT DATE(scl.created_at) FROM tms_check_in_log AS scl WHERE scl.staff_id = ci.staff_id ORDER BY scl.created_at ASC LIMIT 1)'),
        );
        // echo $get;
        // die;
...
// echo $select;
    ...
    function getSalesLeave($staff_id, $start_date, $end_date)
    {
        ...
        $get = array(
            ...
            'joined_at' => 'sf.joined_at',
            'first_check_in' => new Zend_Db_Expr('(SELECT DATE(sl.created_at) FROM tms_leave_log AS sl WHERE sl.staff_id = sf.id ORDER BY sl.created_at ASC LIMIT 1)'),
        );


        $select = $db->select()
            ->from(array('sl' => 'tms_leave_log'), $get)
    ...
    function CheckLeave($staff_id, $date_current)
    {
        ...
        $select = $db->select()
            ->from(array('scl' => 'tms_leave_log'), $get)
 ...
 function loadTmsReportExcelAllCheckIn($area_id, $start_date, $end_date)
    {
        $db = Zend_Registry::get('db');
        ...
        $AAA = $db
            ->select()
            ->from(array('ll' => 'tms_leave_log'), $leave_summary)
...
// die;
        $BBB = $db
            ->select()
            ->from(array('bl' => 'tms_check_in_log'), $check_in_summary)
...
// die;
    ...
    function loadTmsApproveWaitingExcel($area_id, $start_date, $end_date)
    {
        $db = Zend_Registry::get('db');
...
//            'maternity_leave'        => new Zend_Db_Expr("SUM(CASE WHEN pll.action_id = 7 AND pll.status = 'W' THEN DATEDIFF(pll.to_date, pll.from_date)+1 ELSE 0 END)"),
        ...
        $AAA = $db
            ->select()
            ->from(array('pll' => 'tms_leave_log'), $leave_summary)
        ...
        $BBB = $db
            ->select()
            ->from(array('pl' => 'tms_check_in_log'), $check_in_summary)
